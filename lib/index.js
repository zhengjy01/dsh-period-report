...
export const inject = ['tools', 'systemPrompt', 'sessionQuery', 'timer', 'webServer']
...
/** Pending reminder notifications awaiting client acknowledgment. */
const pending = []
...
/** Browser-trust fence: loopback host + same-origin browser markers. */
function isTrusted(req) {
  ...
  const loopback = hn === 'localhost' || hn === '[::1]' || /^127\./.test(hn)
...
/** Read a JSON request body (bounded, never throws). */
function readJson(req) {
  return new Promise((resolve) => {
    let body = ''
    req.on('data', (c) => { body += c; if (body.length > 2e6) body = body.slice(0, 2e6) })
    req.on('end', () => { try { resolve(JSON.parse(body || '{}')) } catch (err) { resolve({}) } })
...
/**
 * Mount the browser API under /dsh-period-report/api (POST JSON):
 *   report  {startDate,endDate,includeSubagents,withUsage,narrate,language}
 *   config  {} (read) | patch object (write)
 *   poll    -> pending reminders
 *   ack     {ids}
 *   test    -> fire one reminder now (needs enabled config)
 */
function mountApi(ctx, store, notify) {
  return ctx.webServer.register({
    kind: 'prefix',
    path: '/dsh-period-report/api',
    handler: async (req, res) => {
      if (!isTrusted(req)) { sendJson(res, 403, { ok: false, error: 'forbidden' }); return }
      if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method not allowed' }); return }
      const pathname = new URL(req.url || '/', 'http://dsh.internal').pathname
      const method = pathname.startsWith('/dsh-period-report/api/') ? pathname.slice('/dsh-period-report/api/'.length) : undefined
      if (method === undefined || method.includes('/')) { sendJson(res, 404, { ok: false, error: 'unknown api' }); return }
      ...
      try {
        switch (method) {
          case 'report': {
            ...
            const out = await buildReport(ctx, startDate, endDate, {
              includeSubagents: !!a.includeSubagents,
              withUsage: a.withUsage !== false,
              narrate: a.narrate !== false,
              language: lang,
            })
            if (!out.ok) { sendJson(res, 200, { ok: false, error: out.error }); return }
            sendJson(res, 200, { ok: true, value: { ...out.report, markdown: renderMarkdown(out.report, lang) } })
            return
          }
          case 'config': {
            ...
            sendJson(res, 200, { ok: true, value: { config: cfg, configPath: CONFIG_FILE } })
            return
          }
          case 'poll': {
            sendJson(res, 200, { ok: true, value: pending.slice() })
            return
          }
          case 'ack': {
            const ids = (body && Array.isArray(body.ids) ? body.ids : []).map(String)
          ...
          case 'test': {
            const cfg = await store.load()
            if (!cfg.enabled) { sendJson(res, 200, { ok: false, error: '提醒未启用，请先在设置页启用' }); return }
            const fired = await fireReminder(ctx, store, notify)
            sendJson(res, 200, { ok: true, value: { fired: fired ? [fired.label] : [] } })
            return
          }
          default: sendJson(res, 404, { ok: false, error: 'unknown api method' })
        }
      } catch (err) {
...
async function fireReminder(ctx, store, notify) {
  ...
  pending.push({ id: 'rem:' + Date.now(), label: res.report.label, language: cfg.language, report: res.report, markdown: md })
...
export function apply(ctx, config = {}) {
  ...
  const sync = () => {
    ...
    if (disposeApi !== undefined) { disposeApi(); disposeApi = undefined }
    if (!enabled) return
    ...
    disposeApi = ctx.effect(() => mountApi(ctx, store, notify), 'dsh-period-report: /dsh-period-report/api routes')
